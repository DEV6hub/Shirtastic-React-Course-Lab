...
const GraphicPicker = ({selectedGraphic, selectedGraphicColor, onGraphicSelected, onGraphicColorSelected}) => <div className="graphic-options">
  <div className="options-heading">Choose a graphic</div>
  <div className="graphics">
    {
      graphicKeys.map(key => <GraphicOption
  ...
  <ColorPicker
    ...
    onColorSelected={(color) => onGraphicColorSelected({type: GRAPHICS_COLOR_EVENT, data: color})}
  />
</div>;

GraphicPicker.defaultProps = {
  selectedGraphicColor: COLOR_BLACK,
  onGraphicSelected: optionEvent => console.log("Graphic Selected Event needs to be handled: ", optionEvent),
  onGraphicColorSelected: optionEvent => console.log("Graphic Color Selected Event needs to be handled: ", optionEvent)
};

const GraphicOption = ({onGraphicSelected, fileName, isSelected}) => {
  const file = require("../../images/shirt-graphics/" + fileName);
  return <button onClick={() => onGraphicSelected({type: GRAPHICS_EVENT, data: fileName})} className="graphic-option">
